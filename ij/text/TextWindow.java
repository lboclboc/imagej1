...
public class TextWindow extends Frame implements ActionListener, FocusListener, ItemListener {
	...
	*/
	public TextWindow(String title, String headings, String data, int width, int height) {
		...
		Point loc=null;
		int w=0, h=0;
		if (title.contains("Results") || title.contains("Measurements")) {
	...
	*/
	...
	void addMenuBar() {
		...
		if (getTitle().contains("Results") || getTitle().contains("Measurements")) {
		...
		mb.add(m);
		if (getTitle().contains("Results") || getTitle().contains("Measurements")) {
			if (getTitle().contains("Measurements")) m = new Menu("Options"); else m = new Menu("Results");
